...
class SunPosition
{
	...
	public function calcSunRiseTransitSet()
	{
		...
		$aα = $α[0] - $α[-1];
		if (abs($aα) >= 2.0) $aα = $this->_limitFromZeroToOne($aα);
		
		$bα = $α[1] - $α[0];
		if (abs($bα) >= 2.0) $bα = $this->_limitFromZeroToOne($bα);
		
		$cα = $bα - $aα;
		
		$aδ = $δ[0] - $δ[-1];
		if (abs($aδ) >= 2.0) $aδ = $this->_limitFromZeroToOne($aδ);
		
		$bδ = $δ[1] - $δ[0];
		if (abs($bδ) >= 2.0) $bδ = $this->_limitFromZeroToOne($bδ);
		...
		foreach ($m as $DayIndex => $mVal) {
			...
			if($this->DEBUG) $this->SunHourNuPrime[$DayIndex] = $ν;
			if($this->DEBUG) $this->SunHourAlphaPrime[$DayIndex] = $α´°;
			if($this->DEBUG) $this->SunHourDeltaPrime[$DayIndex] = $δ´°;
	...
	{
		$timezone = 0.0;
		if($local) $timezone = (float) $this->Observer->ObserverTime->Timezone;
		if($daylightsaving) $timezone+=1.0;
	...
	private function _convertHoursToTime($hours,$withMillisec=false)
	{
		...
		if(!$withMillisec) $seconds = (int) $seconds;
    ...
    private function _calculateEarthHeliocentricLongitude($JME)
    {
        ...
        if($this->DEBUG) $this->TEST_UNIT_L = $EHL;
    ...
    private function _calculateEarthHeliocentricLatitude($JME)
    {
        ...
        if($this->DEBUG) $this->TEST_UNIT_B = $EHB;
    ...
    private function _calculateEarthRadiusVector($JME)
    {
		...
		if($this->DEBUG) $this->TEST_UNIT_R = $R;
